export class Validator {
    ...
    _getAllValues(inputs: NodeListOf<HTMLInputElement>) {
        ...
        if (this.msgArea) {
            const { name } = this.msgArea;
            let { value } = this.msgArea;
    ...
    isValid() {
        ...
        Object.keys(this.values).forEach((inputName) => {
    ...
    checkInput(inputName: string, value: string) {
        ...
        if (inputName === "login") {
            ...
            || this._checkValidity(inputName, value, EMAIL_REGEXP);
        }
    ...
    _toggleErrorMsg(inputName: string, isValid: boolean) {
        if (inputName !== "msg") {
            ...
            if (errMsg) {
                ...
                throw Error("One of the inputs doesn't have an error msg");
            }
        }
    }
